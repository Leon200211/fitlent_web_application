...
if(!empty($_POST['db']) and !empty($_POST['table']) and !empty($_POST['count_pol']) and !empty($_POST['after'])){
    $db = $_POST['db'];
    $table = $_POST['table'];
    $count_pol = $_POST['count_pol'];
    $after = $_POST['after'];
} else if(!empty($_GET['db']) and !empty($_GET['table']) and !empty($_GET['count_pol']) and !empty($_GET['after']) and !empty($_GET['error'])){
    $db = $_GET['db'];
    $table = $_GET['table'];
    $count_pol = $_GET['count_pol'];
    $after = $_GET['after'];
    $error = $_GET['error'];
}
...
<div class="main_body">
    ...
    <div style="margin-left: 200px;    overflow: auto;">
        ...
        <h2>Добавление поля(ей) в таблицу: <?= $table ?> после <?=$after?></h2>
        ...
        <form action="table_structure_add_main.php" method="post">
            <div>
                <table border="1">
                    <tr>
                        <th> Имя </th>
            ...
            <input name="count_pol" value="<?= $count_pol ?>" hidden>
            <input name="after" value="<?= $after ?>" hidden>
